...
function getMapping() {
    ...
    return {
        path, zMin, pz,
        mapx: (x) => ((x - xMin) / px - 0.5) * scale,
        mapy: (y) => ((y - yMin) / py - 0.5) * scale,
        mapz: (z) => ((z - zMin) / pz) * 2
    };
}
// 曲面点云：仅在优化结果变化时重建
...
function buildPath() {
    ...
    if (animPath.length > 1) {
        ...
        for (const pt of animPath)
            pts.push(m.mapx(pt.x), m.mapz(pt.z), m.mapy(pt.y));
    ...
    marker(m.mapx(first.x), m.mapy(first.y), m.mapz(first.z), 0x4fc3f7, 0.14);
    const cur = animPath[animPath.length - 1];
    marker(m.mapx(cur.x), m.mapy(cur.y), m.mapz(cur.z), 0x66bb6a, 0.12);
    const last = m.path[m.path.length - 1];
    marker(m.mapx(last.x), m.mapy(last.y), m.mapz(last.z), 0xef5350, 0.14);
}
// 容器尺寸变化时同步相机宽高比与渲染尺寸
